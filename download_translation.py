...
RESOURCES_URL = f"{BASE_URL}/resources/translation"
DOWNLOAD_DIR = "quran_translations"
MIN_DELAY = 2
MAX_DELAY = 5

# Updated to include the new tags you found
GENERIC_TAGS = [
    "Translation", 
    "With footnotes", 
    "Word by Word", 
...
def main():
    ...
    print(f"Scanning {len(rows)} entries (including Word by Word)...\n")

    for row in rows:
        title_elem = row.find('p', class_='tw-font-semibold')
        if not title_elem: continue
        
        entry_name = sanitize_name(title_elem.text)
        ...
        final_json_path = os.path.join(lang_dir, f"{entry_name}.json")

        # 2. Skip if already downloaded
        if os.path.exists(final_json_path):
            continue

        # 3. FIX: Updated Regex to catch "Download json" AND "Download simple.json"
        ...
        if json_link_elem and 'href' in json_link_elem.attrs:
            ...
            print(f"[NEW] {combined_folder_name} -> {entry_name}")
            ...
            try:
                ...
                with zipfile.ZipFile(io.BytesIO(zip_response.content)) as z:
                    ...
                    if json_files:
                        ...
                        print(f"  -> Success. Resting for {wait_time:.1f}s...")
                        time.sleep(wait_time)
                    else:
                        print(f"  ! Warning: No JSON found inside zip for {entry_name}")
                        
            except Exception as e:
                print(f"  ! Error: {e}")
                time.sleep(5)

    print("\nSync complete! Word by word resources should now be included.")
